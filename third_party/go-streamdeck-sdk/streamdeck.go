...
func (sd *StreamDeck) SetImage(context string, bts []byte) error {
	...
	return sd.SetImageDataURL(context, fmt.Sprintf("data:image/png;base64, %s", b64))
}

// SetImageDataURL dynamically changes the image displayed by an instance of an action using a data URL payload.
func (sd *StreamDeck) SetImageDataURL(context string, image string) error {
